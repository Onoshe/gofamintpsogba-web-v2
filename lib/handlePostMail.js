import { baseUrlPhp, postRequest, siteDataUrl } from "./api";
...
export const handlePostMail = async ({ form }) => {
    ...
    const html = `
        <div>
            <p>Dear GOFAMINT PS Ogba,</p>
            <div>
                ${form.message}
            </div>

            <p>From: ${form.firstname} ${form.lastName}, </p>
    ...
    const subject = "Message from A Website Visitor";
    ...
    return postRequest(siteDataUrl, postBody);
};

/*fetch(baseUrlPhp + "/sendMailManager.php", {
    method: "POST",
    headers: {
